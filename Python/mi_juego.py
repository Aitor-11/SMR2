...
while QUIERO_SEGUIR_JUGANDO:
    ...
    if teclas[pygame.K_c]:
        ...
        AZUL = (rand<Dom.randint(0,255), random.randint(0,255), random.randint(0,255))

    # Aumentar tamaño con T
    if teclas[pygame.K_t]:
                new_width  = rect.width  + SIZE_STEP
                new_height = rect.height + SIZE_STEP

                # Solo aumentar si NO se sale de la ventana
        if rect.1 + new_width <= WIDTH and rect.1 + new_height <= HEIGHT:
        rect.width  = new_width
        rect.height = new_height

            # Disminuir tamaño con Y
    if teclas[pygame.K_y]:
        new_width  = rect.width  - SIZE_STEP
        new_height = rect.height - SIZE_STEP

                # Evitar tamaño negativo o cero
